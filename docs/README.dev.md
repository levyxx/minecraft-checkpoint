...
# JAR をビルド（各 plugin-*/target/ に JAR が生成される）
mvn clean package
...
common/
  ...
  src/main/java/checkpoint/
    ...
    model/
      ...
      RenameResult.java              # リネーム操作の結果 enum
    manager/
    ...
    gui/
      GuiConstants.java              # GUI 定数（スロット番号・タイトル名セット等）
      ItemFactory.java               # CP 関連 ItemStack 生成
      PlayerItemFactory.java         # プレイヤー関連 ItemStack 生成
      MenuManager.java               # 全 GUI 状態管理ファサード
      MenuRenderer.java              # メニュー表示（インベントリ構築）
      MenuClickHandler.java          # インベントリクリック処理
      ChatInputHandler.java          # チャット入力処理
      TeleportHandler.java           # テレポート・CP 操作実行
    i18n/
      Messages.java                  # 多言語メッセージ管理（JP / EN）
    listener/
      InventoryClickListener.java    # インベントリクリックイベントハンドラ
      ChatInputListener.java         # チャット入力イベントハンドラ
      PlayerListener.java            # アイテム操作・ドロップ防止・言語検出
    storage/
...
各 `plugin-{version}/src/main/resources/plugin.yml` の `version` フィールドは `${project.version}` というプレースホルダーになっています。  
Maven のリソースフィルタリング機能によってビルド時に展開されます。

**`plugin.yml` を編集した後は必ず以下を実行してください。**
