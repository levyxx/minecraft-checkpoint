...
| Command | Description |
|---------|-------------|
| `/cp set <name> [description]` | Save current location as a named CP (description optional) |
| `/cp update <name>` | Overwrite existing CP coordinates with current location |
| `/cp delete <name>` | Delete a CP |
| `/cp rename <old> <new>` | Rename a CP |
| `/cp description <name> <desc>` | Set a CP description (spaces allowed) |
| `/cp items` | Receive checkpoint utility items |
| `/cp language <jp\|en>` | Switch display language to Japanese / English |
...
- Override manually with `/cp language <jp|en>`
...
6. Run `/cp language jp` to switch to Japanese
