...
public class CheckpointCommand implements TabExecutor {
    ...
    public CheckpointCommand(CheckpointPluginBase plugin, CheckpointManager checkpointManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (subCommand) {
            ...
            case "did":
                handlers.handleDid(player, playerId);
                break;
            case "didnt":
                handlers.handleDidnt(player, playerId);
                break;
            case "items":
                plugin.giveCheckpointItems(player);
                break;
            case "language":
            case "lang": {
                if (args.length < 2) {
            ...
            case "help":
                handlers.sendHelp(player, playerId, label);
                break;
            default:
                sendUsage(player, playerId, label);
                break;
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            return Arrays.asList("set", "update", "delete", "rename", "description", "items", "did", "didnt", "language", "help").stream()
        ...
        if (args.length >= 2 && "set".equalsIgnoreCase(args[0])) {
            ...
            if (findDescriptionFlag(args) < 0 && last.startsWith("-")) {
                return Arrays.asList("-d", "--description").stream()
        ...
        if (args.length >= 2 && "rename".equalsIgnoreCase(args[0])) {
            if (findNameFlag(args) < 0) {
                String last = args[args.length - 1].toLowerCase(Locale.ROOT);
                if (last.startsWith("-")) {
                    return Arrays.asList("-n", "--name").stream()
                        .filter(opt -> opt.startsWith(last))
                        .collect(Collectors.toList());
                }
                List<String> names = new ArrayList<>(checkpointManager.getNamedCheckpointNames(player.getUniqueId()));
