...
public final class Messages {
    ...
    public static String itemDiamondLore(UUID id) { return get(id, "右クリック: チェックポイント一覧", "Right-click: Checkpoint list"); }
    ...
    public static String emptyHint2(UUID id)        { return get(id, "登録後にダイヤモンドをクリックすると表示されます", "Click Diamond after registering"); }
    ...
    public static String sortOrderLabel(UUID id, checkpoint.model.SortOrder order) {
        switch (order) {
            case NAME_ASC:      return sortNameAsc(id);
            case NAME_DESC:     return sortNameDesc(id);
            case CREATED_ASC:   return sortCreatedAsc(id);
            case CREATED_DESC:  return sortCreatedDesc(id);
            case UPDATED_ASC:   return sortUpdatedAsc(id);
            case UPDATED_DESC:  return sortUpdatedDesc(id);
            case DISTANCE_ASC:  return sortDistanceAsc(id);
            default:            return sortNameAsc(id);
        }
    }
    ...
    public static String playerSortOrderLabel(UUID id, checkpoint.model.PlayerSortOrder order) {
        switch (order) {
            case NAME_ASC:           return pSortNameAsc(id);
            case NAME_DESC:          return pSortNameDesc(id);
            case CLONED_BY_ME_DESC:  return pSortClonedByMe(id);
            case CLONED_COUNT_DESC:  return pSortClonedCount(id);
            case DISTANCE_ASC:       return pSortDistanceAsc(id);
            case LAST_ACTIVITY_DESC: return pSortActivityDesc(id);
            case LAST_ACTIVITY_ASC:  return pSortActivityAsc(id);
            default:                 return pSortNameAsc(id);
        }
    }
    ...
    public static String clearSortOrderLabel(UUID id, checkpoint.model.ClearSortOrder order) {
        switch (order) {
            case NONE:             return clearSortNone(id);
            case CLEARED_FIRST:    return clearSortClearedFirst(id);
            case UNCLEARED_FIRST:  return clearSortUnclearedFirst(id);
            default:               return clearSortNone(id);
        }
    }
}
