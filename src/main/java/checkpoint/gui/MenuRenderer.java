...
class MenuRenderer {
            ...
            for (int col = 1; col <= 7; col++) {
                ...
                if (dataIndex < playerList.size()) {
                    ...
                    inv.setItem(slot, PlayerItemFactory.createPlayerSelectHead(
                        ...
                        totalClonedCount, nearestDist, lastActivityStr, mgr.targetUuidKey));
                }
                itemIndex++;
            }
        }
    ...
    void openCpOperationMenu(Player viewer, String cpName, UUID targetId) {
        ...
        if (isSelf) {
            inv.setItem(9,  PlayerItemFactory.createOperationWoolItem(viewerId, Material.GREEN_WOOL,
                ChatColor.GREEN + Messages.opTeleport(viewerId), Messages.opTeleportLore(viewerId)));
            inv.setItem(11, PlayerItemFactory.createOperationWoolItem(viewerId, Material.BLUE_WOOL,
                ChatColor.AQUA + Messages.opUpdate(viewerId), Messages.opUpdateLore(viewerId)));
            inv.setItem(13, PlayerItemFactory.createOperationWoolItem(viewerId, Material.YELLOW_WOOL,
                ChatColor.YELLOW + Messages.opRename(viewerId), Messages.opRenameLore(viewerId)));
            inv.setItem(15, PlayerItemFactory.createOperationWoolItem(viewerId, Material.ORANGE_WOOL,
                ChatColor.GOLD + Messages.opDescChange(viewerId), Messages.opDescChangeLore(viewerId)));
            inv.setItem(17, PlayerItemFactory.createOperationWoolItem(viewerId, Material.RED_WOOL,
            ...
            inv.setItem(12, PlayerItemFactory.createOperationWoolItem(viewerId, Material.GREEN_WOOL,
                ChatColor.GREEN + Messages.opTeleport(viewerId), Messages.opTeleportLore(viewerId)));
            inv.setItem(14, PlayerItemFactory.createOperationWoolItem(viewerId, Material.CYAN_WOOL,
